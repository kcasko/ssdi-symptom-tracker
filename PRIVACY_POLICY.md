...
- **No advertising networks** or third-party SDKs
...
2. **Health Logs**
   - Daily symptom entries (symptoms, severity, notes)
   - Activity logs (activities, duration, impact)
   - Functional limitations
   - Medication names and schedules
   - Medical appointment details
   - Photo attachments (stored in app's private directory)
   - Voice recordings (if you use voice logging)

3. **Generated Reports**
   - Report drafts and edits
   - Export history metadata (local only)

4. **Evidence Mode Data** (if enabled)
   - Evidence timestamps
   - Log finalization records
   - Revision history
   - Submission pack metadata
...
**Storage Location**: Your device's private app directory, inaccessible to other apps.
...
- **Secure sharing**: Use secure methods to share with healthcare providers
...
|------------|---------|-----------|
| Camera | Take photos of symptom evidence | Optional |
| Photo Library | Attach existing photos to logs | Optional |
| Microphone | Voice logging feature | Optional |
| Biometric | Unlock app with fingerprint/face ID | Optional |
| Storage | Save exported reports to device | Optional |

**All permissions are optional.** Core logging functionality works without any permissions.

## Children's Privacy

This app is not intended for children under 13. We do not knowingly collect information from children. Since the app is local-only, no data is transmitted to us regardless of user age.
...
**All States**: Since we don't collect data, state-specific privacy rights regarding data collection, sale, or sharing do not apply. You have absolute control over your data on your device.
...
- The app is a personal tool, not a medical device
...
**Your privacy is protected because we simply don't have your data.**
